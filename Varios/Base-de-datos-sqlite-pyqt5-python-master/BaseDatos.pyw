...
class Dialogo(QDialog):
 ...
 def Insertar(self):
  estado = self.db.open()
  if estado == False:
    QMessageBox.warning(self, "Error 1", self.db.lastError().text(), QMessageBox.Discard)
  else:
    nombre = self.txt_nombre.text()
    edad = self.txt_edad.text()
   # sql = "INSERT INTO tbusu(nombre, edad) VALUES (:nombre, :edad)"
    consulta = QSqlQuery()
    consulta.prepare("INSERT INTO tbusu(nombre, edad) VALUES (:nombre, :edad)")
  ...
  if estado == True:
   ...
   QMessageBox.warning(self, "Error 2", self.db.lastError().text(), QMessageBox.Discard)
